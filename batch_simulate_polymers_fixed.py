#!/usr/bin/env python3
"""
Fixed batch simulation script for all polymers.
Handles non-contiguous node indices in graph structures.
"""
...
from run_polymer_fixed import run_polymer_simulation
...
def main():
    parser = argparse.ArgumentParser(description='Batch MD simulations for polymers (with fix for non-contiguous indices)')
    ...
    print(f"NOTE: Using fixed script that handles non-contiguous node indices")
    ...
    # If there were failures, show the range of affected molecules
    if len(successful) < len(df_results):
        failed = df_results[df_results['success'] == False]
        print(f"\nFailed molecule IDs: {sorted(failed['mol_id'].tolist())[:10]}{'...' if len(failed) > 10 else ''}")
        if 306 <= failed['mol_id'].min() and failed['mol_id'].max() <= 965:
            print("NOTE: Most failures are in the range 306-965, which have non-contiguous node indices.")
            print("The fixed script should handle these cases correctly.")
